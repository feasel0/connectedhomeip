...
void CommonCheckAbortAllButOneExchange(TestContext * pContext, bool dropResponseMessages)
{
    ...
    auto & sessionManager = pContext->GetSecureSessionManager();
    ...
    CHIP_ERROR err = sessionManager.InjectCaseSessionWithTestKey(
        session1, 100, 101, pContext->GetAliceFabric()->GetNodeId(), pContext->GetBobFabric()->GetNodeId(),
        pContext->GetAliceFabricIndex(), pContext->GetBobAddress(), CryptoContext::SessionRole::kInitiator, {});

    EXPECT_EQ(err, CHIP_NO_ERROR);

    SessionHolder session1Reply;
    err = sessionManager.InjectCaseSessionWithTestKey(session1Reply, 101, 100, pContext->GetBobFabric()->GetNodeId(),
                                                      pContext->GetAliceFabric()->GetNodeId(), pContext->GetBobFabricIndex(),
                                                      pContext->GetAliceAddress(), CryptoContext::SessionRole::kResponder, {});
    ...
    err = sessionManager.InjectCaseSessionWithTestKey(session2, 200, 201, pContext->GetAliceFabric()->GetNodeId(),
                                                      pContext->GetBobFabric()->GetNodeId(), pContext->GetAliceFabricIndex(),
                                                      pContext->GetBobAddress(), CryptoContext::SessionRole::kInitiator, {});

    EXPECT_EQ(err, CHIP_NO_ERROR);

    SessionHolder session2Reply;
    err = sessionManager.InjectCaseSessionWithTestKey(session2Reply, 101, 100, pContext->GetBobFabric()->GetNodeId(),
                                                      pContext->GetAliceFabric()->GetNodeId(), pContext->GetBobFabricIndex(),
                                                      pContext->GetAliceAddress(), CryptoContext::SessionRole::kResponder, {});
    EXPECT_EQ(err, CHIP_NO_ERROR);

    auto & exchangeMgr = pContext->GetExchangeManager();
    ...
    auto & loopback = pContext->GetLoopback();
    ...
    ReliableMessageMgr * rm = pContext->GetExchangeManager().GetReliableMessageMgr();
    ...
    if (dropResponseMessages)
    {
        ...
        pContext->GetIOContext().DriveIOUntil(waitTimeout, [&]() { return false; });
    }
    else
    {
        pContext->DrainAndServiceIO();
...
TestContext * TestAbortExchangesForFabric::mpContext = nullptr;

// Test aborting all but one exchange
TEST_F(TestAbortExchangesForFabric, CheckAbortAllButOneExchange)
{
    CommonCheckAbortAllButOneExchange(mpContext, false);
}

// Test aborting all but one exchange + response timeout
TEST_F(TestAbortExchangesForFabric, CheckAbortAllButOneExchangeResponseTimeout)
{
    CommonCheckAbortAllButOneExchange(mpContext, true);
}
